...
wchar_t* DDIC::Download::Files::download_file(const wchar_t* s_url, const wchar_t* d_file, CallbackNull callback, bool lzma_compressed)
{
    if (d_file == nullptr) 
    {
        std::wstring url(s_url);
        size_t lastSlashPos = url.find_last_of(L'/');
    ...
    std::filesystem::path d_file_path(d_file);
    d_file_path = d_file_path.parent_path();

    if (!DDIC::Download::Files::_CreateDirectoryIfNotExists(d_file_path))
    {
    ...
    {
        // Handle error, destination file already exists and is not a directory
        std::string url = Additionals::Convectors::ConvertWcharPtrToString(s_url);
        ...
        d_file = Additionals::Convectors::ConvertStringToWcharPtr(url);
    }

    HRESULT downloadResult = URLDownloadToFileW(NULL, s_url, d_file, NULL, &callback);

    if (downloadResult == S_OK) 
    {
        std::wstring savedMessage = L"The file is saved as: " + Additionals::Convectors::ConvertWcharPtrToWstring(d_file) + L"\n";
        callback.OnProgress(NULL, NULL, NULL, savedMessage.c_str());
        return const_cast<wchar_t*>(d_file); // OK to return const_cast since the function signature is not const
    }
    else 
    {
        std::wstring errorMessage =
            L"Unable to Download the file: " + Additionals::Convectors::ConvertWcharPtrToWstring(s_url) + 
            L"\nto: " + Additionals::Convectors::ConvertWcharPtrToWstring(d_file);

        callback.OnProgress(NULL, NULL, NULL, errorMessage.c_str());
        return nullptr;
    }
...
std::string DDIC::Download::Files::_get_java_path(wchar_t* dir)
{
