...
{
	// FIXME: make local?
	return QDir::cleanPath(basePath + QDir::separator() + relativePath);
}
...
MetaEntryPtr HttpMetaCache::resolveEntry(QString base, QString resource_path, QString expected_etag)
{
	...
	QString real_path = QDir::cleanPath(selected_base.base_path + QDir::separator() + resource_path);
