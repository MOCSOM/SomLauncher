...
{
    std::shared_ptr<LaunchTask> proc = std::make_shared<LaunchTask>(inst);
    proc->init();
    return proc;
}
...
std::shared_ptr<LogModel> LaunchTask::getLogModel()
{
    if (!this->m_logModel)
    {
        this->m_logModel.reset(std::make_shared<LogModel>().get());
