...
void Yggdrasil::login(QString password)
{
    ...
    req.insert("username", this->m_data->userName());
    ...
    this->m_data->generateClientTokenIfMissing();
    req.insert("clientToken", m_data->clientToken());
...
void Yggdrasil::processResponse(QJsonObject responseData)
{
    ...
    if (this->m_data->clientToken().isEmpty())
    {
        this->m_data->setClientToken(clientToken);
    }
    else if (clientToken != this->m_data->clientToken())
    {
    ...
    this->m_data->yggdrasil_token.token = accessToken;
    this->m_data->yggdrasil_token.validity = Katabasis::Validity::Certain;
    this->m_data->yggdrasil_token.issueInstant = QDateTime::currentDateTimeUtc();
...
void Yggdrasil::processError(QJsonObject responseData)
{
    ...
    {
        this->m_error = std::make_shared<Error>(
            Error
            {
                errorVal.toString(""),
...
void Yggdrasil::processReply()
{
    ...
    default:
        changeState(
            AccountTaskState::STATE_FAILED_SOFT,
            tr("Authentication operation failed due to a network error: %1 (%2)").arg(this->m_netReply->errorString()).arg(this->m_netReply->error())
        );
        return;
    }
    ...
    QByteArray replyData = this->m_netReply->readAll();
    QJsonDocument doc = QJsonDocument::fromJson(replyData, &jsonError);
    // Check the response code.
    int responseCode = this->m_netReply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
    ...
    else 
    {
        ...
        changeState(
            AccountTaskState::STATE_FAILED_SOFT,
            tr("An unknown error occurred when trying to communicate with the authentication server: %1").arg(this->m_netReply->errorString())
        );
    }
}
...
{
    this->timeout_keeper.stop();
    this->timeout_keeper.start(this->timeout_max);
    progress(this->count = 0, this->timeout_max);
}

void Yggdrasil::heartbeat()
{
    this->count += this->time_step;
    progress(this->count, this->timeout_max);
}
...
{
    progress(this->timeout_max, this->timeout_max);
    // TODO: actually use this in a meaningful way
    this->m_aborted = Yggdrasil::BY_TIMEOUT;
...
bool Yggdrasil::abort()
{
    progress(this->timeout_max, this->timeout_max);
    // TODO: actually use this in a meaningful way
    this->m_aborted = Yggdrasil::BY_USER;
    this->m_netReply->abort();
    return true;
}
...
void Yggdrasil::sendRequest(QUrl endpoint, QByteArray content)
{
    ...
    this->m_netReply = APPLICATION->network()->post(netRequest, content);
    connect(this->m_netReply, &QNetworkReply::finished, this, &Yggdrasil::processReply);
    connect(this->m_netReply, &QNetworkReply::uploadProgress, this, &Yggdrasil::refreshTimers);
    connect(this->m_netReply, &QNetworkReply::downloadProgress, this, &Yggdrasil::refreshTimers);
    connect(this->m_netReply, &QNetworkReply::sslErrors, this, &Yggdrasil::sslErrors);
    this->timeout_keeper.setSingleShot(true);
    this->timeout_keeper.start(this->timeout_max);
    this->counter.setSingleShot(false);
    this->counter.start(this->time_step);
    progress(0, this->timeout_max);
    connect(&this->timeout_keeper, &QTimer::timeout, this, &Yggdrasil::abortByTimeout);
    connect(&this->counter, &QTimer::timeout, this, &Yggdrasil::heartbeat);
}