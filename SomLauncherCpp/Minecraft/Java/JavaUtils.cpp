        ...
        if (numSubKeys > 0)
        {
            for (DWORD i = 0; i < numSubKeys; i++)
            {
                ...
                if (retCode == ERROR_SUCCESS)
                {
                    ...
                    QString newKeyName = keyName + "\\" + QString::fromWCharArray(subKeyName, 255) + subkeySuffix;

                    HKEY newKey;
                    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, newKeyName.toStdWString().c_str(), 0,
                        KEY_READ | KEY_WOW64_64KEY, &newKey) == ERROR_SUCCESS)
                    {
                        // Read the JavaHome value to find where Java is installed.
                        value = new char[0];
                        valueSz = 0;
                        if (RegQueryValueEx(newKey, keyJavaDir.toStdWString().c_str(), NULL, NULL, (BYTE*)value,
                            &valueSz) == ERROR_MORE_DATA)
                        {
                            value = new char[valueSz];
                            RegQueryValueEx(newKey, keyJavaDir.toStdWString().c_str(), NULL, NULL, (BYTE*)value,
                                &valueSz);

                            // Now, we construct the version object and add it to the list.
                            JavaInstallPtr javaVersion = std::make_shared<JavaInstall>();

                            javaVersion->id = QString::fromWCharArray(subKeyName, 255);
                            javaVersion->arch = archType;
                            javaVersion->path =
                                QDir(std::filesystem::path(std::string(value), std::string("bin")).u8string().c_str()).absoluteFilePath("javaw.exe");
    ...
    auto scanJavaDir = [&](const QString& dirPath)
        {
            ...
            for (auto& entry : entries)
            {
                ...
                javas.append(std::filesystem::path(prefix, "jre/bin/java"));
                javas.append(std::filesystem::path(prefix, "bin/java"));
            }
        };
    // oracle RPMs
