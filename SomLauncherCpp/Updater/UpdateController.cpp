...
void UpdateController::installUpdates()
{
    ...
    QString finishCmd = FS::PathCombine(m_root, LAUNCHER_NAME);
...
#error Unsupported operating system.
    ...
    QString backupPath = QString::fromStdWString((std::filesystem::path(m_root.toStdWString()) / "update" / "backup").wstring());
    QDir origin(m_root);

    // clean up the backup folder. it should be empty before we start
    if (!std::filesystem::remove(backupPath.toStdWString()))
    {
        qWarning() << "couldn't remove previous backup folder" << backupPath;
    }
    // and it should exist.
    if (!std::filesystem::exists(backupPath.toStdWString()))
    {
    ...
    for (auto& op : m_operations)
    {
        ...
        {
#ifdef Q_OS_WIN32
            QString windowsExeName = LAUNCHER_NAME + ".exe";
...
#endif
            QFileInfo destination(std::filesystem::path(m_root.toStdWString()) / op.destination.toStdWString());
#ifdef Q_OS_WIN32
            
            if (/*QSysInfo::windowsVersion() < QSysInfo::WV_VISTA*/ false)
            {
                if (destination.fileName() == windowsExeName)
                {
                    ...
                    exeBackup = rootDir.relativeFilePath(QString::fromStdWString(std::filesystem::path(backupPath.toStdWString()) / destination.fileName().toStdWString()));
                    useXPHack = true;
                    continue;
                }
            }
            ...
            if (destination.exists())
            {
                ...
                QString backupFilePath = QString::fromStdWString(std::filesystem::path(backupPath.toStdWString()) / backupName.toStdWString());
            ...
            if (!std::filesystem::exists(destination.absoluteFilePath().toStdWString()))
            {
        ...
        {
            QString destFilePath = QString::fromStdWString(std::filesystem::path(m_root.toStdWString()) / op.destination.toStdWString());
            if (QFile::exists(destFilePath))
            {
                QString backupName = op.destination;
                backupName.replace('/', '_');
                QString trashFilePath = QString::fromStdWString(std::filesystem::path(backupPath.toStdWString()) / backupName.toStdWString());
    ...
    if (useXPHack)
    {
        ...
        QString scriptPath = QString::fromStdWString(std::filesystem::path(m_root.toStdWString()) / "update" / "update.vbs");
    ...
    if (!startFailed && doLiveCheck)
    {
        ...
        while (attempts < 10)
        {
            ...
            try
            {
                key = QString::fromUtf8(liveCheckFile);
                //auto id = QApplication::applicationPid();
...
void UpdateController::fail()
{
    ...
    case Replace:
    {
        ...
        ).arg(m_failedFile, LAUNCHER_NAME);
        doRollback = true;
    ...
    if (doRollback)
    {
        ...
        if (!rollbackOK)
        {
            msg = QObject::tr("The rollback failed too.\n"
                ...
                "Please let us know why and how this happened.").arg(LAUNCHER_NAME);
